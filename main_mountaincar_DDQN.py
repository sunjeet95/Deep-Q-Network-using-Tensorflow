...
# This code is for Training and Testing the "Mountain-Car-v0" problem in Open Gym
...
Replay_Memory=25000					#Max Size of Replay Memory
Epsilon_decay=0.0001				#Epsilon decay
...
class Network():
	...
	def __init__(self, input, scope_name, Batch_Size=1):
		...
		with tf.variable_scope(scope_name) as scope:	#Creating the Scope
			...
			self.first_fully_connected_1=tf.layers.dense(inputs=self.input, units=100,activation=tf.nn.relu,kernel_initializer= tf.truncated_normal_initializer (),
			...
			with tf.variable_scope("Fully_Connected_Layer_2"):	#Creating the Scope for Output layer 
				...
				self.first_fully_connected_2=tf.layers.dense(inputs=self.first_fully_connected_1, units=100,activation=tf.nn.relu,kernel_initializer= tf.truncated_normal_initializer (),
															 bias_initializer=tf.initializers.ones(),name="Fully_Connected_Layer_2")	#Fully Connected Operation
			
	
			self.output_layer=tf.layers.dense(inputs=self.first_fully_connected_2, units=3,kernel_initializer= tf.truncated_normal_initializer (), 
...
with tf.device('/device:GPU:0'):
	...
	x_Target_Values=tf.placeholder(dtype=tf.float32,shape=(None,2), name="x_Target_Values") 		# Creating place holder for giving state as input to the network
	...
	x_Eval_Net=tf.placeholder(dtype=tf.float32,shape=(None,2), name="x_Eval_Net") 		# Creating place holder for giving state as input to the network
...
def main():
	...
	env = gym.make('MountainCar-v0').env		# Making the environment for MountainCar-v0 from Open AI Gym
